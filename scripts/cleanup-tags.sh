...
# Удаляет временные Git-теги (dev, beta, all) для указанного пакета.
# Использование:
#   cleanup-tags.sh <package-path> <type> [branch-pattern]
...
if [ -z "$PKG" ] || [ -z "$TYPE" ]; then
    echo "Usage: cleanup-tags.sh <package-path> <type> [branch-pattern]"
    exit 1
fi
